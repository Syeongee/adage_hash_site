...
var hash_apply = function(){
    if($hash_txt.length > 0){ //hash로 받아온 값이 존재한다면
        ...
        $contGroup = ""; //sub.html 내부에서 상단 메뉴 클릭시, 기존에 누적된 쌓인 데이터들을 모두 재초기화
...
//var 약점은 기존에 작성된 변수명이 있다고 하더라도 새롭게 동일한 명칭으로 변수명을 잡게되면 갱신(데이터의 완전 교체)이 됨. const로 선언된 변수명은 고정값만을 가져올 수 있음(갱신 불가 - 견고함, 경직된 부분, 객제, 배열, 함수)
